...
public class GuiGunModTable extends GuiContainer 
{
	...
	protected void drawGuiContainerForegroundLayer(int x, int y)
    {
		...
		if(gunStack != null && gunStack.getItem() instanceof ItemGun)
		{
			...
			//fontRendererObj.drawString(String.valueOf(roundFloat(gunType.getSpread(gunStack), 2)), 241, 74, 0x404040); before goldsloth
			fontRendererObj.drawString(String.valueOf((gunType.getSpread(gunStack, false, false))), 241, 74, 0x404040);
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) 
	{
		...
		if(gunStack != null && gunStack.getItem() instanceof ItemGun)
		{
			...
			//int[] stats = { Math.round(gunType.getDamage(gunStack)) * 4, Math.round(gunType.getSpread(gunStack)) * 4, before goldsloth
