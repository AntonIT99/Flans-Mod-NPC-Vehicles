...
{
	public int disTime;
	public static ResourceLocation icon = new ResourceLocation("flansmod", "particle/overkill.png");
	public EntityOverKill(World w, double px, double py, double pz, double mx, double my, double mz)
	{
		...
		this.disTime = 10;
		icon = new ResourceLocation("flansmod", "particle/overkill.png");
	}
    ...
    public void renderParticle(Tessellator par1Tessellator, float par2, float par3, float par4, float par5, float par6, float par7)
    {
    	...
    	FMLClientHandler.instance().getClient().renderEngine.bindTexture(new ResourceLocation("flansmod", "particle/overkill.png"));

        float scale = 0.1F + (this.particleAge * 0.4F);
        float xPos = (float) (this.prevPosX + (this.posX - this.prevPosX) * (double) par2 - interpPosX);
        float yPos = (float) (this.prevPosY + 5 + (this.posY - this.prevPosY) * (double) par2 - interpPosY);
        ...
        par1Tessellator.setColorRGBA_F(this.particleRed * (colorIntensity - this.particleAge*0.02F), this.particleGreen * (colorIntensity - this.particleAge*0.05F), this.particleBlue * (colorIntensity - this.particleAge*0.05F), (1F - this.particleAge*this.particleAge*this.particleAge*0.0081F));
