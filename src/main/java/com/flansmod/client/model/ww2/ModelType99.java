package com.flansmod.client.model.ww2;

import com.flansmod.client.model.ModelMG;
import com.flansmod.client.tmt.ModelRendererTurbo;

public class ModelType99 extends ModelMG
{
	int textureX = 64;
	int textureY = 32;
	
	public ModelType99()
	{
		bipodModel = new ModelRendererTurbo[2];
		bipodModel[0] = new ModelRendererTurbo(this, 0, 0, textureX, textureY);
		bipodModel[1] = new ModelRendererTurbo(this, 0, 0, textureX, textureY);
		
		gunModel = new ModelRendererTurbo[3];
		gunModel[0] = new ModelRendererTurbo(this, 0, 0, textureX, textureY);
		gunModel[1] = new ModelRendererTurbo(this, 4, 0, textureX, textureY);
		gunModel[2] = new ModelRendererTurbo(this, 0, 18, textureX, textureY);
		
		ammoModel = new ModelRendererTurbo[2];
		ammoModel[0] = new ModelRendererTurbo(this, 4, 5, textureX, textureY);
		ammoModel[1] = new ModelRendererTurbo(this, 4, 5, textureX, textureY);

		
		bipodModel[0].addBox(0F, 0F, 0F, 1, 8, 1, 0F);
		bipodModel[0].rotateAngleZ = 3.14159265F / 4F;
		bipodModel[0].setRotationPoint(5.66F, 0F, 0F);
		bipodModel[1].addBox(-1F, 0F, 0F, 1, 8, 1, 0F);
		bipodModel[1].rotateAngleZ = -3.14159265F / 4F;
		bipodModel[1].setRotationPoint(-5.66F, 0F, 0F);
		
		gunModel[0].addBox(-1F, -1F, -2F, 2, 2, 16, 0F);
		gunModel[0].setRotationPoint(0F, 6F, 0F);
		
		gunModel[1].addBox(-1F, -4F, 8F, 2, 3, 2, 0F);
		gunModel[1].setRotationPoint(0F, 6F, 0F);
		
		gunModel[2].addBox(-0.5F, -0.5F, -8F, 1, 1, 6, 0F);
		gunModel[2].setRotationPoint(0F, 6F, 0F);
		
		ammoModel[0].addBox(0F, 1F, 5F, 1, 3, 2, 0F);
		ammoModel[0].setRotationPoint(0F, 6F, 0F);
		
		ammoModel[1].addBox(0F, 4F, 4.5F, 1, 2, 2, 0F);
		ammoModel[1].setRotationPoint(0F, 6F, 0F);
		
	}
}
