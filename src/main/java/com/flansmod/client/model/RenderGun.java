	...
	private void renderGun(ItemRenderType type, ItemStack item, GunType gunType, GunAnimations animations,
		...
		GL11.glPushMatrix();
		{
			...
			case EQUIPPED_FIRST_PERSON:
			{
				...
				if (animations.switchAnimationProgress > 0 && animations.switchAnimationLength > 0) { //goldsloth gundraw
...
//mount and blade labjac				
	...
	private static void preRenderAttachment(AttachmentType attachment, ItemStack stack, Vector3f model, GunType type) {
	...
	private static void postRenderAttachment(AttachmentType attachment, ItemStack stack, float f)
	{
		Paintjob paintjob = attachment.getPaintjob(stack.getItemDamage());
	...
	private static void renderAttachmentAmmo(float f, AttachmentType grip, ModelGun model, Paintjob ammo, Paintjob otherAmmo)
	{
...
//	}
	...
	private static void renderFirstPersonArm(EntityPlayer player, ModelGun model, GunAnimations anim) {
		Minecraft mc = Minecraft.getMinecraft();
...
//                             System.out.println(socks.type.armourTextureName);
	...
	private static void renderAnimArm(EntityPlayer player, ModelGun model, GunType type, GunAnimations anim) 
...
//                             System.out.println(socks.type.armourTextureName);
	...
	private static float getEndLoadedDistance(AttachmentType grip, GunType gun, ItemStack gunStack)
	{
	 ...
	 */
	//TODO
	private static float getNumBulletsInReload(GunAnimations animations, AttachmentType grip, GunType gun, ItemStack gunStack)
	{
	...
	private static float getRecoilDistance(AttachmentType grip, GunType gun, ItemStack gunStack)
	{
		if (grip != null && gun.getSecondaryFire(gunStack))
	...
	private static float getRecoilAngle(AttachmentType grip, GunType gun, ItemStack gunStack)
	{
		if (grip != null && gun.getSecondaryFire(gunStack))
