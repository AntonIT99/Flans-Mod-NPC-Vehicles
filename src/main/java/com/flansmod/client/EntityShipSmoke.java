...
public class EntityShipSmoke extends EntityFX
{
    ...
    public void renderParticle(Tessellator par1Tessellator, float par2, float par3, float par4, float par5, float par6, float par7)
    {
        ...
        float scale = 1.0F + (this.particleAge * 0.1F);
        float xPos = (float) (this.prevPosX + 0.1*(this.posX - this.prevPosX) * (double) par2 - interpPosX);
        float yPos = (float) (this.prevPosY + this.particleAge*0.3f/(1 + 0.01f*this.particleAge) + 0.1*(this.posY - this.prevPosY) * (double) par2 - interpPosY);
        float zPos = (float) (this.prevPosZ - this.particleAge*0.01f*(Math.random() - 0.5f) + (this.posZ - this.prevPosZ) * (double) par2 - interpPosZ);
        float colorIntensity = 0.1F; //darker grey
        ...
        par1Tessellator.setColorRGBA_F(this.particleRed * colorIntensity, this.particleGreen * colorIntensity, this.particleBlue * colorIntensity, (1F - this.particleAge*this.particleAge*this.particleAge*0.000001F));
