...
{
	public int disTime;
	public static ResourceLocation icon = new ResourceLocation("flansmod", "particle/Blood.png");
    ...
    public void renderParticle(Tessellator par1Tessellator, float par2, float par3, float par4, float par5, float par6, float par7)
    {
    	...
    	FMLClientHandler.instance().getClient().renderEngine.bindTexture(new ResourceLocation("flansmod", "particle/Blood.png"));

        float scale = 0.3F + (this.particleAge * 0.03F);
        float xPos = (float) (this.prevPosX + (this.posX - this.prevPosX) * 0.001f * (double) par2 - interpPosX);
        float yPos = (float) (this.prevPosY + (this.posY - this.prevPosY) * 0.001f * (double) par2 - interpPosY);
        float zPos = (float) (this.prevPosZ + (this.posZ - this.prevPosZ) * 0.001f * (double) par2 - interpPosZ);
        ...
        par1Tessellator.setColorRGBA_F(this.particleRed * (colorIntensity - this.particleAge*0.005F), this.particleGreen * (colorIntensity - this.particleAge*0.05F), this.particleBlue * (colorIntensity - this.particleAge*0.05F), (1F - this.particleAge*this.particleAge*this.particleAge*0.0081F));
